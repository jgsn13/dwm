...
void arrangemon(Monitor *m) {
  ...
  if (m->lt[m->sellt]->arrange)
    ...
    /* <>< case; rather than providing an arrange function and upsetting other
     * logic that tests for its presence, simply add borders here */
    for (c = selmon->clients; c; c = c->next)
      if (ISVISIBLE(c) && c->bw == 0)
        resize(c, c->x, c->y, c->w - 2 * borderpx, c->h - 2 * borderpx,
               borderpx, 0);
}
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    w = TEXTW(tags[i]);
    drw_setscheme(drw, (m->tagset[m->seltags] & 1 << i ? tagschemesel[i]
                                                       : tagschemenorm[i]));
...
void tile(Monitor *m) {
  ...
  if (n > m->nmaster)
    ...
    mw = m->ww - m->gap->gappx;
  for (i = 0, my = ty = m->gap->gappx, c = nexttiled(m->clients); c;
       c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      h = (m->wh - my) / (MIN(n, m->nmaster) - i) - m->gap->gappx;
      resize(c, m->wx + m->gap->gappx, m->wy + my, mw - 2 * bw - m->gap->gappx,
             h - 2 * bw, bw, 0);
      if (my + HEIGHT(c) + m->gap->gappx < m->wh)
        my += HEIGHT(c) + m->gap->gappx;
    } else {
      h = (m->wh - ty) / (n - i) - m->gap->gappx;
      resize(c, m->wx + mw + m->gap->gappx, m->wy + ty,
             m->ww - mw - 2 * bw - 2 * m->gap->gappx, h - 2 * bw, bw, 0);
...
void togglefloating(const Arg *arg) {
  ...
  if (selmon->sel->isfloating)
    resize(selmon->sel, selmon->sel->x, selmon->sel->y,
           ...
           selmon->sel->h - 2 * (borderpx - selmon->sel->bw), borderpx, 0);
  arrange(selmon);
}
